...
- **Instacart Shopping List Checkout:** Server-side `POST /api/instacart/shopping-list` endpoint creates an Instacart shopping list link via `createInstacartShoppingListLink()` in `server/lib/instacartMeasurement.ts`. Calls `POST connect.instacart.com/idp/v1/products/products_link` with `link_type: "shopping_list"` and `line_items` array (name/quantity/unit/display_text). Safety fallbacks: null/empty unit → "each", null/<=0 qty → 1. Strict response validation reads `products_link_url` from response. Frontend sends simplifiedLineItems (name/qty/unit/displayText) from the existing canonicalization+purchase-decision pipeline, redirects to returned `productsLinkUrl` via `window.location.assign()`. Standardized response shape: `{success, checkoutMethod:"redirect", productsLinkUrl, correlationId, correlationIds}`. Logging: `instacart_shopping_list_request_built` (server), `instacart_api_response` (server+client) with fields success/checkoutMethod/redirectUrlGenerated/productsLinkUrl.
