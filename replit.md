...
- **Canonical Recipe shape:** `{ id, title, image, cuisine, sub_category, dish_type, prepTime, cookTime, totalTime, servings, calories, protein, carbs, fat, mealTypes, cookingStyle, ingredients: [{name, amount, unit}], steps }`. `sub_category` is `null` when DB stores "NULL" string or null. `dish_type` uses a 30-item classification array matching Supabase values.
