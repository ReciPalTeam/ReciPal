...
- **User Preferences Management:** Floating Save button in filter Sheet that persists preferences (Time/Difficulty, Cost, Dietary, Allergies) to user profile. Changes require explicit Save action (no auto-save). Save shows "Preferences saved" toast and refreshes For You feed only. Dirty state tracking compares current values against saved profile values. Collapsible filter sections with proper test IDs for E2E testing.
