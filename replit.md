...
- **Recipe Diversity:** Meal-type-specific variety keyword arrays (BREAKFAST_VARIETY, LUNCH_VARIETY, DINNER_VARIETY, etc.) with diverse proteins (ground beef, steak, salmon, shrimp, pork, turkey, lamb, fish, tofu) and food types (birria tacos, curry, casserole, stew, meatballs, burgers). Three variety keywords searched simultaneously per page load with daily rotation offset and feed-type offset (+5 for Something New) to prevent same-protein dominance. For You ranking by pantryFitScore remains unaffected by diversity changes.
