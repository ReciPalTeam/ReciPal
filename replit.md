...
- **Instacart Recipe Page Checkout:** Server-side `POST /api/instacart/recipe-page` endpoint creates an Instacart Recipe Page via their official API (`POST connect.instacart.com/idp/v1/products/recipe`). Uses `decideInstacartMeasurement()` in `server/lib/instacartMeasurement.ts` to decide per-ingredient whether to include measurement. Measurement policy: (1) Spices & Seasonings → omit (spice_container); (2) Produce+cup → omit (produce_recipe_volume); (3) serving unit → omit; (4) parenthetical/juice-of display → omit (unsupported_unit_display); (5) low confidence+non-safe unit → omit; (6) "each" → include with fractional rounding; weight/volume (ounce/pound/gram/kg/ml/liter/fl_oz) → include as-is; else → omit. Items without measurement send name-only to Instacart. Bulletproof URL extraction tries 12 response field paths. Frontend redirects to returned URL via `window.location.assign()`. Logging: `instacart_recipe_page_request_built` (server), `instacart_recipe_page_api_response` (server), `instacart_recipe_page_redirect` (client).
