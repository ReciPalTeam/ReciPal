...
- **Photo Entry:** Orange "+" menu → "Photo Entry" opens native camera (mobile) or image upload (web). Client-side image compression to ~1024px JPEG. Backend `POST /api/fatsecret/image-recognition` sends base64 to FatSecret image recognition API. Meal card displays detected foods with remove capability and macro totals. Two questions: Q1 "Was this from pantry?" Q2 "Save to My Recipes?". Confirm logs macros via `POST /api/consumption-logs` and optionally saves recipe via `POST /api/custom-recipes`. Components: `PhotoEntrySheet` (`client/src/components/photo-entry-sheet.tsx`).
