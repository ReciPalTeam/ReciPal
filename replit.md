...
- **Meal Planning:** Automated weekly plan generation, flexible scheduling. "Auto-populate Week" uses a batched per-meal-type fetch protocol: `GET /api/recipes/feed/planner?meal_type=X&offset=N&limit=7&exclude=ids` fetches 7 recipes per Supabase meal_type ('Breakfast', 'Lunch', 'Dinner', 'Dessert', 'Snack/Appetizer') per batch. No "fetch 200 most recent" — each batch is targeted. Client maintains `seenRecipeIds` and `batchOffsets` refs per meal_type per session to prevent repeats. Regenerate fetches the NEXT 7 per type. Free users: `filterRecipes` applies allergy/diet exclusions. Pro users: additionally `applyProHardLimits` enforces per-meal calorie+macro hard caps (daily target ÷ 3). Iterative fetch loop (up to 10 iterations) fills batch if post-filtering shrinks results. `generateWeekPlan` in `client/src/lib/auto-populate.ts` accepts `candidateRecipes: Recipe[]` parameter and is fully pure (no global state). `buildUserPrefs()` reads real profile data (allergies, dietaryPreferences, cookingComfort, macroGoals for Pro, calorieGoal for Free). Macro-aware scoring: `scoreRecipe` tracks running daily totals per slot and penalizes recipes that push a day's total above Pro targets (`targetCalories`/`targetProtein`/`targetCarbs`/`targetFat`) or Free `calorieGoal`. Locked meals' macros are pre-loaded into daily totals so new selections respect existing nutrition. Past dates are disabled. Preview overlay always generates a full week; existing planner meals appear as locked-by-default. Lock/Unlock (green 3D gloss) button per slot prevents regeneration overwrite. Red X button removes unlocked meals from preview. Regenerate respects locked meals. Confirm Plan replaces changed meals in the planner. `addToPlanner` in `demo-store.ts` uses `useRecipeStore.getState().recipesById` for recipe lookup (no mockRecipes dependency), ensuring tracker sync works for Supabase recipes.
...
- **Recipe Interaction:** Recipe discovery with client-side filtering, Supabase-backed feeds and search, and database-backed favorites. Pantry-aware recipe cards display "Have X • Maybe Y • Need Z" status. "For You" feed ranks recipes by `pantryFitScore`. All recipe data (feeds, search, detail) served exclusively from Supabase via server-only endpoints (`/api/recipes/feed/for-you`, `/api/recipes/feed/something-new`, `/api/recipes/search`, `/api/recipes/:recipeId`, `/api/recipes/shared/:recipeId`). FatSecret is used ONLY for Manual Entry ingredient search and barcode scanner. No mockRecipes fallback on Recipes page, recipe detail, or share page.
...
- **Canonical Recipe shape:** `{ id, title, image, cuisine, sub_category, dish_type, prepTime, cookTime, totalTime, servings, calories, protein, carbs, fat, mealTypes, cookingStyle, ingredients: [{name, amount, unit}], steps }`. `sub_category` is `null` when DB stores "NULL" string or null. `dish_type` uses a 30-item classification array matching Supabase values. `mealTypes` are passed through directly from Supabase `meal_type` column ('Breakfast', 'Lunch', 'Dinner', 'Dessert', 'Snack/Appetizer'). `mapRecipeToMealType` in `auto-populate.ts` maps these to slot names: Breakfast, Lunch, Dinner, Desserts, Snackitizers.
